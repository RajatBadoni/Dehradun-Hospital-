...
/**
 * Seeds doctors + a default admin account.
 * By default this is idempotent: it only seeds doctors if the doctors
 * table is currently empty, and only creates the admin if that email
 * doesn't already exist. Pass { force: true } to always reset doctors
 * (used by the manual `npm run seed` command).
 */
async function seedDatabase({ force = false } = {}) {
  const existingDoctors = db.get("doctors").value();
  if (force || !existingDoctors || existingDoctors.length === 0) {
